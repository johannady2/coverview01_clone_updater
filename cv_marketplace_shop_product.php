...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_shop_product&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_shop_product (id,id_shop,id_product,marketplace_seller_id_product)VALUES(".$id[$i].",".$id_shop[$i].",".$id_product[$i].",".$marketplace_seller_id_product[$i].");",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_shop_product WHERE idNOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_shop_product`;";
