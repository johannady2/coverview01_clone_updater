...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_product_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_product_lang WHERE (id_product,id_shop,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/12)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_product_lang(id_product,id_shop,id_lang,description,description_short,link_rewrite,meta_description,meta_keywords,meta_title,name,available_now,available_later)VALUES(".$id_product[$i].",".$id_shop[$i].",".$id_lang[$i].",'".$description[$i]."','".$description_short[$i]."','".$link_rewrite[$i]."','".$meta_description[$i]."','".$meta_keywords[$i]."','".$meta_title[$i]."','".$name[$i]."','".$available_now[$i]."','".$available_later[$i]."');",$conn);
...
if($id_product == NULL )
{
	$deletesql = "DELETE FROM `cv_product_lang`;";
}
