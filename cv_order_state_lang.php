...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_order_state_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_order_state_lang WHERE (id_order_state,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_order_state_lang(id_order_state,id_lang,name,template)VALUES(".$id_order_state[$i].",".$id_lang[$i].",'".$name[$i]."','".$template[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM `cv_order_state_lang`;";
}
