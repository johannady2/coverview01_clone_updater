...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_seller_orders&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_seller_orders (id,id_customer_seller,seller_shop,total_earn,total_admin_commission,date_add,date_upd)VALUES(".$id[$i].",".$id_customer_seller[$i].",'".$seller_shop[$i]."',".$total_earn[$i].",".$total_admin_commission[$i].",'".$date_add[$i]."','".$date_upd[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_seller_orders WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_seller_orders`;";
