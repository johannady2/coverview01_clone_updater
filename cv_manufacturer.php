...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_manufacturer&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/3)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_manufacturer (id_manufacturer,name,date_add,date_upd,active)VALUES(".$id_manufacturer[$i].",'".$name[$i]."','".$date_add[$i]."','".$date_upd[$i]."','".$active[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM cv_manufacturer WHERE id_manufacturer NOT IN (".$id_manufacturers_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_manufacturer`;";
