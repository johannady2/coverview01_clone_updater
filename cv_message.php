...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_message&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
$arr = array();
...
for($i = 0 ; $i <= (sizeof($arr)/8)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_message (id_message,id_cart,id_customer,id_employee,id_order,message,private,date_add)VALUES(".$id_message[$i].",".$id_cart[$i].",".$id_customer[$i].",".$id_employee[$i].",".$id_order[$i].",'".$message[$i]."',".$private[$i].",'".$date_add[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM cv_message WHERE id_message NOT IN (".$id_messages_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_message`;";
