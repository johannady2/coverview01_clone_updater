...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_category_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_category_lang WHERE (id_category,id_shop,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/12)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_category_lang(id_category,id_shop,id_lang,name,description,link_rewrite,meta_title,meta_keywords,meta_description)VALUES(".$id_category[$i].",".$id_shop[$i].",".$id_lang[$i].",'".$name[$i]."','".$description[$i]."','".$link_rewrite[$i]."','".$meta_title[$i]."','".$meta_keywords[$i]."','".$meta_description[$i]."');",$conn);
...
if($id_category == NULL )
{
	$deletesql = "DELETE FROM `cv_category_lang`;";
}
