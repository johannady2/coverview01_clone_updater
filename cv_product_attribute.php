...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_product_attribute&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/16)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_product_attribute (id_product_attribute,id_product,reference,supplier_reference,location,ean13,upc,wholesale_price,price,ecotax,quantity,weight,unit_price_impact,default_on,minimal_quantity,available_date)VALUES(".$id_product_attribute[$i].",".$id_product[$i].",'".$reference[$i]."','".$supplier_reference[$i]."','".$location[$i]."','".$ean13[$i]."','".$upc[$i]."',".$wholesale_price[$i].",".$price[$i].",".$ecotax[$i].",".$quantity[$i].",".$weight[$i].",".$unit_price_impact[$i].",".$default_on[$i].",".$minimal_quantity[$i].",'".$available_date[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM cv_product_attribute WHERE id_product_attribute NOT IN (".$id_product_attributes_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_product_attribute`;";
