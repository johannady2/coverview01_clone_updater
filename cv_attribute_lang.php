...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_attribute_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_attribute_lang WHERE (id_attribute,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/3)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_attribute_lang(id_attribute,id_lang,name)VALUES(".$id_attribute[$i].",".$id_lang[$i].",'".$name[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM `cv_attribute_lang`;";
}
