...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_attribute_impact&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/5)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_attribute_impact (id_attribute_impact,id_product,id_attribute,weight,price)VALUES(".$id_attribute_impact[$i].",".$id_product[$i].",".$id_attribute[$i].",".$weight[$i].",".$price[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_attribute_impact WHERE id_attribute_impact NOT IN (".$id_attribute_impacts_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_attribute_impact`;";
