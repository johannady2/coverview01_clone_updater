...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_category_group&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_category_group WHERE (id_category,id_group) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/12)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_category_group(id_category,id_group)VALUES(".$id_category[$i].",".$id_group[$i].");",$conn);
...
if($id_category == NULL )
{
	$deletesql = "DELETE FROM `cv_category_group`;";
}
