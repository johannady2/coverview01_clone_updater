...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_order_history&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/5)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_order_history(id_order_history,id_employee,id_order,id_order_state,date_add)VALUES(".$id_order_history[$i].",".$id_employee[$i].",".$id_order[$i].",".$id_order_state[$i].",'".$date_add[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM cv_order_history WHERE id_order_history NOT IN (".$id_order_historys_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_order_history`;";
