...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_delivery&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_delivery (id,order_id,delivery_date,received_by)VALUES(".$id[$i].",".$order_id[$i].",'".$delivery_date[$i]."','".$received_by[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_delivery WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_delivery`;";
