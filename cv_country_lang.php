...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_country_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_country_lang WHERE (id_country,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/3)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_country_lang(id_country,id_lang,name)VALUES(".$id_country[$i].",".$id_lang[$i].",'".$name[$i]."');",$conn);
...
if($id_country == NULL )
{
	$deletesql = "DELETE FROM `cv_country_lang`;";
}
