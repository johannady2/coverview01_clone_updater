...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_payment_mode&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
foreach($arr as $key => $value)
{
	foreach($value as $as => $info)
	{
		...
		else if($as == 'payment_mode')
		{
			array_push($payment_mode, addslashes($info));
		}
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_address (id,payment_mode)VALUES(".$id[$i].",'".$payment_mode[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_address WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_address`;";
