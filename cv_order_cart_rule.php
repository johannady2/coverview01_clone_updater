...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_order_cart_rule&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
{
	//$selectquery = "SELECT *  FROM cv_product WHERE id_product =".$id_product[$i];
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_order_cart_rule(id_order_cart_rule,id_order,id_cart_rule,id_order_invoice,name,value_tax_excl,free_shipping)VALUES(".$id_order_cart_rule[$i].",".$id_order[$i].",".$id_cart_rule[$i].",".$id_order_invoice[$i].",'".$name[$i]."',".$value_tax_excl[$i].",".$free_shipping[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_order_cart_rule WHERE id_order_cart_rule NOT IN (".$id_order_cart_rules_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_order_cart_rule`;";
