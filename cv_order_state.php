...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_order_state&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
{
	//$selectquery = "SELECT *  FROM cv_order_state WHERE id_order_state =".$id_order_state[$i];
	...
	//echo $id_order_state[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_order_state(id_order_state,invoice,send_email,module_name,color,unremovable,hidden,logable,delivery,shipped,paid,pdf_invoice,pdf_delivery,deleted)VALUES(".$id_order_state[$i].",".$invoice[$i].",".$send_email[$i].",'".$module_name[$i]."','".$color[$i]."',".$unremovable[$i].",".$hidden[$i].",".$logable[$i].",".$delivery[$i].",".$shipped[$i].",".$paid[$i].",".$pdf_invoice[$i].",".$pdf_delivery[$i].",".$deleted[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_order_state WHERE id_order_state NOT IN (".$id_order_states_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_order_state`;";
