...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_category_product&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_category_product WHERE (id_category,id_product,position) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/12)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_category_product(id_category,id_product,position)VALUES(".$id_category[$i].",".$id_product[$i].",".$position[$i].");",$conn);
...
if($id_category == NULL )
{
	$deletesql = "DELETE FROM `cv_category_product`;";
}
