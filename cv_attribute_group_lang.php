...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_attribute_group_lang&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
$deletesql = "DELETE FROM cv_attribute_group_lang WHERE (id_attribute_group,id_lang) NOT IN (";
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	if(mysql_error($conn) != "")
	{
		...
		mysql_query("REPLACE INTO coverview01db_clone.cv_attribute_group_lang(id_attribute_group,id_lang,name,public_name)VALUES(".$id_attribute_group[$i].",".$id_lang[$i].",'".$name[$i]."','".$public_name[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM `cv_attribute_group_lang`;";
}
