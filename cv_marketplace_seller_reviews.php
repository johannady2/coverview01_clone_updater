...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_seller_reviews&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_seller_reviews (id_review,id_seller,id_customer,customer_email,rating,review,active,date_add)VALUES(".$id_review[$i].",".$id_seller[$i].",".$id_customer[$i].",'".$customer_email[$i]."',".$rating[$i].",".$review[$i].",".$active[$i].",'".$date_add[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_seller_reviews WHERE id_review NOT IN (".$id_reviews_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_seller_reviews`;";
