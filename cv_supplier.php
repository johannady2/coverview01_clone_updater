...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_supplier&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/5)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_supplier(id_supplier,name,date_add,date_upd,active)VALUES(".$id_supplier[$i].",'".$name[$i]."','".$date_add[$i]."','".$date_upd[$i]."',".$active[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_supplier WHERE id_supplier NOT IN (".$id_suppliers_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_supplier`;";
