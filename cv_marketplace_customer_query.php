...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_customer_query&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/10)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_customer_query (id,id_product,id_customer,id_seller,subject,description,customer_email,active,date_add,date_upd)VALUES(".$id[$i].",".$id_product[$i].",".$id_customer[$i].",".$id_seller[$i].",'".$subject[$i]."','".$description[$i]."','".$customer_email[$i]."',".$active[$i].",'".$date_add[$i]."','".$date_upd[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_customer_query WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_customer_query`;";
