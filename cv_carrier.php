...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_carrier&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/22)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_carrier (id_carrier,id_reference,id_tax_rules_group,name,url,active,deleted,shipping_handling,range_behavior,is_module,is_free,shipping_external,need_range,external_module_name,shipping_method,position,max_width,max_height,max_depth,max_weight,grade)VALUES(".$id_carrier[$i].",".$id_reference[$i].",".$id_tax_rules_group[$i].",'".$name[$i]."','".$url[$i]."',".$active[$i].",".$deleted[$i].",".$shipping_handling[$i].",".$range_behavior[$i].",".$is_module[$i].",".$is_free[$i].",".$shipping_external[$i].",".$need_range[$i].",'".$external_module_name[$i]."',".$shipping_method[$i].",".$position[$i].",".$max_width[$i].",".$max_height[$i].",".$max_depth[$i].",".$max_weight[$i].",".$grade[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_carrier WHERE id_carrier NOT IN (".$id_carriers_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_carrier`;";
