...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_customer&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/5)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_customer (id,marketplace_seller_id,id_customer,is_seller)VALUES(".$id[$i].",".$marketplace_seller_id[$i].",".$id_customer[$i].",".$is_seller[$i].");",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_customer WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_customer`;";
