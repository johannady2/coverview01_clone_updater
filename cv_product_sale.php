...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_product_sale&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_product_sale (id_product,quantity,sale_nbr,date_upd)VALUES(".$id_product[$i].",".$quantity[$i].",".$sale_nbr[$i].",'".$date_upd[$i]."');",$conn);
...
{
	$deletesql = "DELETE FROM cv_product_sale WHERE id_product NOT IN (".$id_products_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_product_sale`;";
