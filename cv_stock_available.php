...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_stock_available&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
mysql_query("REPLACE INTO coverview01db_clone.cv_stock_available (id_stock_available,id_product,id_product_attribute,id_shop,id_shop_group,quantity,depends_on_stock,out_of_stock)VALUES(".$id_stock_available[$i].",".$id_product[$i].",".$id_product_attribute[$i].",".$id_shop[$i].",".$id_shop_group[$i].",".$quantity[$i].",".$depends_on_stock[$i].",".$out_of_stock[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_stock_available WHERE id_stock_available NOT IN (".$id_stock_availables_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_stock_available`;";
