...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_state&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/24)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_state (id_state,id_country,id_zone,name,iso_code,tax_behavior)VALUES(".$id_state[$i].",".$id_country[$i].",".$id_zone[$i].",'".$name[$i]."','".$iso_code[$i]."',".$tax_behavior[$i].");",$conn);
...
{
	$deletedsql = "deleted FROM cv_state WHERE id_state NOT IN (".$id_states_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_state`;";
