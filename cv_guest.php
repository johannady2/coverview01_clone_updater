...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_guest&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/16)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_guest (id_guest,id_operating_system,id_web_browser,id_customer,javascript,screen_resolution_x,screen_resolution_y,screen_color,sun_java,adobe_flash,adobe_director,apple_quicktime,real_player,windows_media,accept_language,mobile_theme)VALUES(".$id_guest[$i].",".$id_operating_system[$i].",".$id_web_browser[$i].",".$id_customer[$i].",".$javascript[$i].",".$screen_resolution_x[$i].",".$screen_resolution_y[$i].",".$screen_color[$i].",".$sun_java[$i].",".$adobe_flash[$i].",".$adobe_director[$i].",".$apple_quicktime[$i].",".$real_player[$i].",".$windows_media[$i].",'".$accept_language[$i]."',".$mobile_theme[$i].");",$conn);
...
{
	$deletedsql = "deleted FROM cv_guest WHERE id_guest NOT IN (".$id_guests_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_guest`;";
