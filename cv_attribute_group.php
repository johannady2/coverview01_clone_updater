...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_attribute_group&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_attribute_group (id_attribute_group,is_color_group,group_type,position)VALUES(".$id_attribute_group[$i].",".$is_color_group[$i].",'".$group_type[$i]."',".$position[$i].");",$conn);
...
{
	$deletesql = "DELETE FROM cv_attribute WHERE id_attribute_group NOT IN (".$id_attribute_groups_IMPOLODED.")";
}
else
{

	$deletesql = "DELETE FROM `cv_attribute_group`;";
