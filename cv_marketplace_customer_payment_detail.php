...
$data = json_decode(file_get_contents("http://coverview01.com/ForI5/?table=cv_marketplace_customer_payment_detail&glogdbcloneupdater-access=76ef0d45220fdee3ac883a0c7565e50c"),true);
...
for($i = 0 ; $i <= (sizeof($arr)/4)-1 ; $i++)//AllData/columnsPerData
{
	...
	//echo $id_product[$i].'- Numrrows = '.$numrows.'<br>';
			...
			mysql_query("REPLACE INTO coverview01db_clone.cv_marketplace_customer_payment_detail (id,id_customer,payment_mode_id,payment_detail)VALUES(".$id[$i].",".$id_customer[$i].",".$payment_mode_id[$i].",'".$payment_detail[$i]."');",$conn);
...
{
	$deletedsql = "deleted FROM cv_marketplace_customer_payment_detail WHERE id NOT IN (".$ids_IMPOLODED.")";
}
else
{

	$deletedsql = "deleted FROM `cv_marketplace_customer_payment_detail`;";
